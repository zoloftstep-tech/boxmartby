...
**Версия:** 1.2 (актуализация 2026-09-15) · исходный: 1.1 / 28 июля 2026

> **Статус:** частично superseded. Email после CRM ingest на Site **остаётся**. Telegram **заказных** карточек с Site **снят** (Phase H): менеджеры получают заказы из CRM. Site TG = только pricing-contract alert. Актуальный поток: [`HANDOFF.md`](HANDOFF.md) / [`OPS.md`](OPS.md) §«Сайт → CRM».

Этот документ заменял раздел 6 файла `BoxMart-Website-TZ.md` (n8n) и дополнял §3.2 `BoxMart-API-TZ.md`. Ниже — исторический Variant A (Email + TG с Site).

---

## 1. Общая логика

**Исторически:** при `/api/submit-order` — параллельно Telegram Bot API + Gmail SMTP.

**Сейчас:**
1. CRM ingest (SoT заказа) + `Idempotency-Key`.
2. Email через `nodemailer` (best-effort после успешного ingest).
3. Telegram заказных уведомлений — **CRM**, не Site.
